...
const mapRow = (arr) => map(arr, (it) => transform(it, (r, v, k) => (r[camelCase(k)] = v)))
...
const convertCsvToJs = (file) => {
    ...
    writeFileSync(destFile, flow([readFile, parseCsv, mapRow, prettyPrint])(file));
};
...
writeFileSync(resolve(DEST, `index.js`), jsFiles.map((it) => {
